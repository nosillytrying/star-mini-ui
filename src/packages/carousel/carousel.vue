<template>
  ...
  >
    ...
    <div class="arrow-box">
      <star-mini-button icon="left" @click="select(currentSelected - 1,true)"></star-mini-button>
      <star-mini-button icon="right" @click="select(currentSelected + 1,true)"></star-mini-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    run() {
      if (this.autoplay) {
        console.log(this.delay, 'this.delaythis.delaythis.delay')
        // 如果是自动不放 就开始运行
  ...
  mounted() {
    ...
    this.children = this.$children.filter(
      item => item.$options.name == "star-mini-carousel-item"
    );
