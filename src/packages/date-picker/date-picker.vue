<template>
  <div class="star-mini-date-picker" v-click-outside="handleBlur">
    <star-mini-input suffix-icon="rili" @focus="handleFocus" :value="formateDate" @change="handleChange"></star-mini-input>
    <div class="star-mini-date-content" v-if="isVisible">
      <div class="star-mini-date-picker-content">
        <template v-if="mode === 'dates'">
          <div class="star-mini-date-picker-header">
            <star-mini-icon icon="prev" @click="changeYear(-1)"></star-mini-icon>
            <star-mini-icon icon="left" @click="changeMonth(-1)"></star-mini-icon>
            <span>
              <b @click="mode='years'">{{tempTime.year}}</b>年
              <b @click="mode='months'">{{tempTime.month + 1}}</b>月
            </span>
            <star-mini-icon icon="right" @click="changeMonth(1)"></star-mini-icon>
            <star-mini-icon icon="next" @click="changeYear(1)"></star-mini-icon>
          </div>
          <div>
        ...
        <template v-if="mode === 'years'">
            <div class="star-mini-date-picker-header">
            <star-mini-icon icon="prev" @click="changeYear(-10)"></star-mini-icon>
            <span>
              <b @click="mode='years'">{{startYear}}年{{startYear+9}}年</b>
            </span>
            <star-mini-icon icon="next" @click="changeYear(10)"></star-mini-icon>
          </div>
        </template>
        <template v-if="mode === 'months'">
            <div class="star-mini-date-picker-header">
            <star-mini-icon icon="left" @click="changeYear(-1)"></star-mini-icon>
            <span>
              <b @click="mode='years'">{{this.tempTime.year}}年</b>
            </span>
            <star-mini-icon icon="right" @click="changeYear(1)"></star-mini-icon>
          </div>
        </template>
      </div>
    </div>
...
.star-mini-date-picker {
  display: inline-block;
  .star-mini-date-content {
    position: absolute;
    z-index: 10;
    user-select: none;
    width: 280px;
